...
class AccountRepo {
    ...
    public function ListAll($user, $withChildren = false) {
        $accounts = Account::leftJoin('accounts as parent', 'accounts.parent_account_id', '=', 'parent.account_id')
            ...
            ->select(
                ...
                DB::raw('coalesce(billing_address.name, shipping_address.name) as billing_address_name'),
                DB::raw('coalesce(billing_address.formatted, shipping_address.formatted) as billing_address_formatted'),
                DB::raw('coalesce(billing_address.lat, shipping_address.lat) as billing_address_lat'),
                DB::raw('coalesce(billing_address.lng, shipping_address.lng) as billing_address_lng'),
                DB::raw('coalesce(billing_address.place_id, shipping_address.place_id) as billing_address_place_id'),
    ...
    public function ListForBillsPage($user, $withChildren = false, $activeOnly = false) {
