...
class BillCollector {
	public function Collect($req, $permissions, $pickupAddressId, $deliveryAddressId) {
		$collectedBill = [
			...
			'delivery_reference_value' => $req->delivery_address_type == 'Account' ? trim($req->delivery_reference_value) ?? null : null,
			...
			'pickup_reference_value' => $req->pickup_address_type === 'Account' ? trim($req->pickup_reference_value) ?? null : null,
		...
		if((!$req->bill_id && $permissions['createFull']) || (isset($permissions['editDispatch']) && $permissions['editDispatch']))
			$collectedBill = array_merge($collectedBill, [
				...
				'delivery_person_name' => $req->delivery_person_name == "" ? null : $req->delivery_person_name,
				...
				'pickup_person_name' => $req->pickup_person_name == "" ? null : $req->pickup_person_name,
	...
	public function CollectCharges($req, $billId) {
		...
		foreach($req->charges as $charge) {
			...
			$temp = [
				...
				'charge_reference_value' => trim($charge['charge_reference_value']) ?? null,
				'charge_reference_value_label' => $charge['charge_reference_value_label'] ?? null,
